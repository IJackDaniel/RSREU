...
class BinarySearchTree:
    ...
    def find_height(self, node, level):
        if node is None:
            return 0
        return max([level, self.find_height(node.left, level+1), self.find_height(node.right, level + 1)])


b_tree = BinarySearchTree()
string = list(map(int, input().split()))
for elem in string:
    if not b_tree.search(elem) and elem != 0:
        b_tree.insert(elem)
print(b_tree.find_height(b_tree.root, 1))
# 7 3 2 1 9 5 4 6 8 0
